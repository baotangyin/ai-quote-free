...
import { handlers, makeRecognizeHandler, makeAiTestHandler, makeParseTemplateXlsxHandler, makeRecognizeDrawingHandler, makeRecognizeScreenshotHandler, makeRecognizeScreenshotProductHandler } from '../../src/main/ipc';
...
describe('products:recognizeScreenshot', () => {
  it('未配置 AI 时抛「请先在设置中配置 AI 档案」', async () => {
    const db = buildDb();
    await expect(handlers['products:recognizeScreenshot'](db, { image: { mediaType: 'image/png', base64: 'AAAA' } }))
      .rejects.toThrow('请先在设置中配置 AI 档案');
  });
  ...
  it('用「图片处理」用途绑定档案（未设置 visionModel 时透传主 AI 配置的 model）', async () => {
    ...
    configureAi(db);
    const chatFn = vi.fn().mockResolvedValue(
      JSON.stringify({ found: true, name: 'A', brand: null, model: null, category: null, dims: null, unit: null, paramsCore: null, priceYuan: 100, note: null }),
    );
    const handler = makeRecognizeScreenshotProductHandler(chatFn);
    const result = await handler(db, { image: { mediaType: 'image/png', base64: 'AAAA' } });
    expect(chatFn).toHaveBeenCalledTimes(1);
    const cfgArg = chatFn.mock.calls[0][0] as AiConfig;
    expect(cfgArg.model).toBe('test-model');
    expect(result.found).toBe(true);
    expect(result.priceYuan).toBe(100);
  });
  ...
  it('设置 visionModel 后覆盖 model', async () => {
    ...
    handlers['settings:set'](db, { key: 'visionModel', value: 'vision-model-x' });
    const chatFn = vi.fn().mockResolvedValue(
      JSON.stringify({ found: false, name: null, brand: null, model: null, category: null, dims: null, unit: null, paramsCore: null, priceYuan: null, note: null }),
    );
    const handler = makeRecognizeScreenshotProductHandler(chatFn);
    await handler(db, { image: { mediaType: 'image/png', base64: 'AAAA' } });
    const cfgArg = chatFn.mock.calls[0][0] as AiConfig;
  ...
  it('不支持的图片格式抛「不支持的图片格式」', async () => {
    const db = buildDb();
    configureAi(db);
    const handler = makeRecognizeScreenshotProductHandler(vi.fn());
    await expect(handler(db, { image: { mediaType: 'image/gif', base64: 'AAAA' } }))
      .rejects.toThrow('不支持的图片格式');
  });
