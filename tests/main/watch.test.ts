...
import { ensureSettingsTable, setSetting, setAiProfiles, type AiProfile } from '../../src/main/settings';
...
describe('watch:runNow / watch:status 全链路（chat 依赖注入 mock）', () => {
  ...
  it('查价档案 searchMode=dashscope 时，chat 调用的 opts 含 enable_search:true（extraBody 注入查价链路）', async () => {
    const db = buildDb();
    const profile: AiProfile = {
      id: 'p1', name: '查价档案', protocol: 'openai', baseUrl: 'https://api.test',
      apiKey: 'sk-test', model: 'test-model', searchMode: 'dashscope',
    };
    setAiProfiles(db, [profile]);
    setSetting(db, 'aiProfileWatch', profile.id);
    core.createProduct(db, { name: 'X', unit: '台', watchPrice: true });
    const chatFn = vi.fn().mockResolvedValue(
      JSON.stringify({ found: false, priceYuan: null, sourceUrl: null, note: null }),
    );
    ...
    expect(opts.extraBody).toEqual({ enable_search: true });
  });

  it('查价档案 searchMode=none/缺省时，chat 调用的 opts.extraBody 为 undefined', async () => {
    const db = buildDb();
    configureAi(db);
    core.createProduct(db, { name: 'X', unit: '台', watchPrice: true });
    const chatFn = vi.fn().mockResolvedValue(
