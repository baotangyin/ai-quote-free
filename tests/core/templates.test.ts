...
const SECTIONS: TemplateSection[] = [
  { name: '多媒体硬件', integrationFeeRate: 0.05, isHardware: true, linkSpaces: false, spaces: [
    { name: '安防监控系统设备', description: null, pinBottom: true }
  ] },
  { name: '软件影片', integrationFeeRate: 0, isHardware: false, linkSpaces: true, spaces: [] }
];
...
describe('templates repo', () => {
  ...
  it('校验：空板块名/空空间名/负费率拒绝；宽容缺省字段', () => {
    ...
    expect(validateTemplateSections([{ name: 'A' }])).toEqual([
      { name: 'A', integrationFeeRate: 0, isHardware: false, linkSpaces: false, spaces: [] }
    ]);
    expect(validateTemplateSections([{ name: 'A', linkSpaces: true }])[0].linkSpaces).toBe(true);
  });
  ...
  it('applyTemplate：按序建板块+空间，置底/费率/硬件生效；已有板块时追加在末尾', () => {
    ...
    expect(secs[1].linkSpaces).toBe(false);
    expect(secs[2].linkSpaces).toBe(true);
    db.close();
  });
  ...
  it('createProjectWithTemplate：有同名类型模板则自动生成骨架，无模板/无类型则仅建项目', () => {
    ...
    expect(secs1.map((s) => s.linkSpaces)).toEqual([false, true, true]);
