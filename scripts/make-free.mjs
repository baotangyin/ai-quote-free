...
function runMakeFree(wt) {
  ...
  const DELETE_LIST = [
    'src/core/license',
    'src/main/license.ts',
    'src/main/licenseOnline.ts',
    'src/main/usageReporter.ts',
    'tests/main/usage-reporter.test.ts',
    'src/main/priceBrowser.ts',
    'scripts/license-tool.mjs',
    'tests/core/license.test.ts',
    'tests/main/license.test.ts',
    'tests/main/license-online.test.ts',
  ...
  console.log('生成孤儿提交……');
  gitIn(wt, ['add', '-A']);
  // 免费版仓库的提交信息不暴露私库来源或提交号。
  const commitMessage = `AI 报价单 免费版 v${version}`;
  gitIn(wt, ['commit', '-m', commitMessage]);
  console.log(`已生成免费版发布分支（v${version}）。`);
...
function applyPreciseReplacements(wt) {
  ...
  replaceOnce(
    ...
    [
      ...
      '        </Paragraph>',
      '        <Paragraph style={P}>',
      '          <Text strong>在线激活</Text>：也可在「软件授权」卡片输入供应商提供的授权码（格式 AIQ-XXXX-XXXX-XXXX）后点击「激活」，联网换取签名许可，无需重启即可生效。在线授权会在启动后定时联网校验（约每 24 小时一次），也可随时点击「立即校验」手动校验；若<Text strong>连续超过 30 天</Text>无法联网校验，会自动降级为只读模式，联网校验成功后自动恢复。',
      '        </Paragraph>',
    ].join('\n'),
    [
      '        <Paragraph style={P}>',
