...
import {
  ...
  createEstimateNorm, listEstimateNorms, updateEstimateNorm, deleteEstimateNorm, batchCreateEstimateNorms,
...
import { chatComplete, testConnection, fetchModelNames, type AiConfig } from '../core/ai/client';
import { analyzeCompletedProjectCosts, type SpaceCostStats } from '../core/domain/estimate';
...
export function makeParseTemplateXlsxHandler(chatFn: typeof chatComplete = chatComplete) {
  return async (db: Db, payload: { filePath: string }) => {
    ...
    const chat: ChatFn = (messages, opts) => chatFn(cfg, messages, { ...opts, feature: 'template' });
...
export function makeRecognizeHandler(chatFn: typeof chatComplete = chatComplete) {
  return async (db: Db, payload: { sheetName: string; grid: string[][] }) => {
    const cfg = readAiConfig(db);
    const chatWithFeature: typeof chatComplete = (c, messages, opts) =>
      chatFn(c, messages, { ...opts, feature: 'import' });
    return recognizeSheet(cfg, payload.sheetName, payload.grid, { chatFn: chatWithFeature });
  };
}
...
/** `ai:modelNames` handler：按档案信息查询该提供商的官方模型列表。 */
export function makeAiModelNamesHandler(fetchModelNamesFn: typeof fetchModelNames = fetchModelNames) {
  return async (_db: Db, payload: { name: string; protocol: 'openai' | 'anthropic'; baseUrl: string; apiKey: string }) => {
    const cfg: AiConfig = { protocol: payload.protocol, baseUrl: payload.baseUrl, apiKey: payload.apiKey, model: '' };
    return fetchModelNamesFn(cfg);
  };
}

/**
 * `estimate:norms:suggestFromCompleted` handler：聚合已完成项目空间造价 → AI 生成指标建议。
 */
export function makeSuggestNormsFromCompletedHandler() {
  return async (db: Db) => {
    const stats = analyzeCompletedProjectCosts(db);
    if (stats.length === 0) return { stats: [], suggestions: [] };

    // 读取文本识别用途的 AI 档案
    const profile = getAiProfileFor(db, 'aiProfileText');
    if (!profile) throw new Error('请先在设置中配置 AI 档案（文本识别用途）');

    const cfg = profileToAiConfig(profile);
    const statsJson = JSON.stringify(stats.map(s => ({
      projectType: s.projectType,
      spaceName: s.spaceName,
      unitPriceYuanPerSqm: Math.round(s.unitPriceCentsPerSqm / 100) / 100,
      medianYuanPerSqm: Math.round(s.medianCentsPerSqm / 100) / 100,
      p25YuanPerSqm: Math.round(s.p25CentsPerSqm / 100) / 100,
      p75YuanPerSqm: Math.round(s.p75CentsPerSqm / 100) / 100,
      sampleCount: s.sampleCount,
    })));

    const prompt = `你是一个智能化/展厅行业的造价分析师。以下是从已完成项目中统计的空间单位面积造价数据（元/㎡）：

${statsJson}

请分析这些数据并生成概算指标建议。要求：
1. 将相似空间名归类归一化（如"大会议室""小会议室""贵宾会议室"→"会议室"），取合理的单价区间
2. 区间基于数据分布：建议下限取 P25 附近，上限取 P75 附近
3. 样本数 < 3 的空间类型仅作参考，标注"(样本少)"
4. 仅输出 JSON 数组，每项含：projectType(项目类型,可为null)、spaceType(归一后空间类型)、unitPriceLowYuan(下限,元)、unitPriceHighYuan(上限,元)、note(说明,含原始空间名和样本数)
5. 禁止编造不存在的数值，所有数字必须来源于输入数据

输出格式严格为：[{"projectType":"...","spaceType":"...","unitPriceLowYuan":...,"unitPriceHighYuan":...,"note":"..."},...]`;

    const reply = await chatComplete(cfg, [
      { role: 'system', content: '你是造价分析助手。仅输出有效 JSON，不输出解释文字。' },
      { role: 'user', content: prompt },
    ], { maxTokens: 4096 });

    // 提取 JSON
    const jsonMatch = reply.match(/\[[\s\S]*\]/);
    if (!jsonMatch) throw new Error('AI 返回的内容中未找到 JSON 数组');
    const suggestions: { projectType?: string | null; spaceType: string; unitPriceLowYuan: number; unitPriceHighYuan: number; note?: string }[] = JSON.parse(jsonMatch[0]);

    return {
      stats: stats.map(s => ({
        projectType: s.projectType,
        spaceName: s.spaceName,
        unitPriceCentsPerSqm: s.unitPriceCentsPerSqm,
        sampleCount: s.sampleCount,
      })),
      suggestions: suggestions.map(item => ({
        projectType: item.projectType ?? null,
        spaceType: item.spaceType,
        unitPriceLowCents: Math.round((item.unitPriceLowYuan || 0) * 100),
        unitPriceHighCents: Math.round((item.unitPriceHighYuan || 0) * 100),
        note: item.note ?? null,
      })),
    };
  };
...
export function makeRecognizeDrawingHandler(chatFn: typeof chatComplete = chatComplete) {
  return async (db: Db, payload: { images: { mediaType: string; base64: string }[] }) => {
    ...
    const chat: VisionChatFn = (messages, opts) => chatFn(cfg, messages, { ...opts, feature: 'drawing' });
...
export function makeWatchRunNowHandler(chatFn: typeof chatComplete = chatComplete, runNowDeps: RunNowDeps = {}) {
  return async (db: Db) => {
    ...
    const chat: VisionChatFn = (messages, opts) => chatFn(cfg, messages, { ...opts, extraBody, feature: 'watch' });
...
export function makeRecognizeScreenshotHandler(chatFn: typeof chatComplete = chatComplete) {
  return async (db: Db, payload: { image: { mediaType: string; base64: string } }) => {
    ...
    const chat: VisionChatFn = (messages, opts) => chatFn(cfg, messages, { ...opts, feature: 'screenshot' });
...
export function makeRecognizeScreenshotProductHandler(chatFn: typeof chatComplete = chatComplete) {
  return async (db: Db, payload: { image: { mediaType: string; base64: string } }) => {
    ...
    const chat: VisionChatFn = (messages, opts) => chatFn(cfg, messages, { ...opts, feature: 'product' });
...
export const handlers: Record<string, (db: Db, payload: any) => any> = {
  ...
  'ai:modelNames': makeAiModelNamesHandler(),
  ...
  'estimate:norms:batchCreate': (db, payload) => batchCreateEstimateNorms(db, payload),
  'estimate:norms:suggestFromCompleted': makeSuggestNormsFromCompletedHandler(),
