...
import { applyPendingRestore, backupDatabase, stageRestore, validateBackupFile } from './backup';
import { registerIpc, readWatchAiProfile, profileToAiConfig, searchExtraBody, makeWatchRunNowHandler, setAppVersion } from './ipc';
...
import { registerInputContextMenu } from './contextMenu';
...
function buildWatchChat(db: Db): VisionChatFn | null {
  try {
    ...
    return (messages, opts) => chatComplete(cfg, messages, { ...opts, extraBody, feature: 'watch' });
  } catch {
    return null;
  }
}
...
app.whenReady().then(() => {
  ...
  /* istanbul ignore next -- 依赖 electron dialog 对象，涉及真实文件系统弹窗，ipc.test.ts 不覆盖 */
  ipcMain.handle('dialog:pickDbFile', async () => {
    const result = await dialog.showOpenDialog({
      properties: ['openFile'],
  ...
  /* istanbul ignore next -- 依赖真实 electron db.backup 与文件系统写盘，backupDatabase 本身在 backup.test.ts 覆盖 */
  ipcMain.handle('backup:run', async (_event, payload: { destDir: string }) => backupDatabase(db, payload.destDir));

  /* istanbul ignore next -- 依赖 electron app.getPath('userData')，validateBackupFile/stageRestore 本身在 backup.test.ts 覆盖 */
  ipcMain.handle('backup:stageRestore', (_event, payload: { filePath: string }) => {
    const result = validateBackupFile(payload.filePath);
    if (!result.ok) return result;
    stageRestore(payload.filePath, dbPath);
    return { ok: true };
  });

  /* istanbul ignore next -- 依赖真实 electron app.relaunch/app.quit，人工冒烟验证，不纳入自动化测试 */
