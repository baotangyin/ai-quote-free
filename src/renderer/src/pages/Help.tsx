...
const TOPICS: HelpTopic[] = [
  ...
  },
  {
    key: 'products',
    title: '3. 产品库',
    keywords: '产品 分类 标签 参数 选配 成本价 品牌 型号 截图识别 AI提取产品信息',
    body: (
      <>
        <Paragraph style={P}>
        ...
        <Paragraph style={P}>
          <Text strong>截图识别</Text>：新建 / 编辑产品弹窗顶部「截图识别」按钮，粘贴（Ctrl/Cmd+V）或拖入一张产品页截图，AI 提取名称、品牌、型号、分类、规格、单位、核心参数要点等信息，
          <Text strong>仅自动填充当前为空的字段</Text>，不会覆盖你已填写的内容；若同时识别出价格，会显示勾选项「建档后写入价格记录（手工来源）」，默认勾选，保存产品时一并写入该产品的一条价格记录。仅识别用户手动提供的截图，不会自动访问任何网页。
        </Paragraph>
        <Paragraph style={P}>
