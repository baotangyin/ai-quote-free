...
import type { ScreenshotPriceResult } from '../core/ai/screenshotPrice';
import type { ScreenshotProductResult } from '../core/ai/screenshotProduct';
...
export type { ScreenshotPriceResult } from '../core/ai/screenshotPrice';
export type { ScreenshotProductResult } from '../core/ai/screenshotProduct';
...
export interface AiQuoteApi {
  ...
  /**
   * 新建/编辑产品弹窗内「截图识别」：仅识别用户手动截图/拖入的产品页图片，不做任何自动访问
   * 页面的行为；结果不落库，由调用方 form.setFieldsValue 填表，价格识别到时可选调用 pricesAdd 写入。
   */
  productsRecognizeScreenshot(input: { image: { mediaType: string; base64: string } }): Promise<ScreenshotProductResult>;
  ...
  /** 选择数据库备份文件（.db），用于「从备份还原」。 */
  dialogPickDbFile(): Promise<string | null>;
  shellReveal(path: string): Promise<null>;
  /** 用系统默认浏览器打开外部链接（如更新下载页）。 */
  shellOpenExternal(url: string): Promise<null>;

  // 数据备份与还原
  /** 在线备份当前数据库到 destDir，返回生成的备份文件完整路径。 */
  backupRun(input: { destDir: string }): Promise<string>;
  /** 校验并暂存一份备份文件，待下次启动时生效；ok=false 时 reason 为中文失败原因，不做任何改动。 */
  backupStageRestore(input: { filePath: string }): Promise<{ ok: boolean; reason?: string }>;
  /** 重启应用（stageRestore 成功后用户确认「立即重启」时调用，重启过程中完成数据库替换）。 */
  appRelaunch(): Promise<null>;
  ...
  /** 根据提供商信息查询官方模型名称列表，需先填好协议/Base URL/API Key 后调用。 */
  aiModelNames(input: { name: string; protocol: 'openai' | 'anthropic'; baseUrl: string; apiKey: string }): Promise<string[]>;
  ...
  estimateNormsBatchCreate(input: { projectType?: string | null; spaceType?: string | null; unitPriceLowCents?: number | null; unitPriceHighCents?: number | null; note?: string | null }[]): Promise<number>;
  estimateNormsSuggestFromCompleted(): Promise<{ stats: { projectType: string | null; spaceName: string; unitPriceCentsPerSqm: number; sampleCount: number }[]; suggestions: { projectType: string | null; spaceType: string; unitPriceLowCents: number; unitPriceHighCents: number; note: string | null }[] }>;
  ...
  /**
   * 截图识价：仅识别用户手动截图/拖入的商品页图片，不做任何自动访问页面的行为；
   * 识别结果不落库，由调用方在用户确认后走 pricesAdd 写入。
   */
  watchRecognizeScreenshot(input: { image: { mediaType: string; base64: string } }): Promise<ScreenshotPriceResult>;
